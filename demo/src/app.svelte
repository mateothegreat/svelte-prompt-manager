<script lang="ts">
  import { Button, Dialog, DialogClose, DialogContent, DialogDescription, DialogFooter, DialogHeader, DialogTitle, DialogTrigger, InnerDialog, InnerDialogContent, InnerDialogDescription, InnerDialogFooter, InnerDialogHeader, InnerDialogTitle, InnerDialogTrigger, Input, Label, RadioGroup, RadioGroupItem } from "@mateothegreat/new";
  ...
  <Dialog>
    ...
    <DialogContent class="p-0">
      <DialogHeader class="border-b p-4">
        ...
        <DialogDescription>Please enter your credit card credentials below to complete the payment process.</DialogDescription>
      </DialogHeader>
      ...
      <DialogFooter class="flex flex-col items-center justify-between space-y-2 border-t px-4 py-2 sm:flex-row sm:space-y-0">
        <InnerDialog>
          ...
          <InnerDialogContent class="-mt-6 p-0 sm:-mt-1">
            ...
            <div class="flex flex-col gap-4 p-4">
              <RadioGroup value={selectedPaymentMethod} onValueChange={(value) => (selectedPaymentMethod = value)}>
                <div class={`hover:bg-accent flex cursor-pointer items-center justify-between rounded-lg border p-4 ${selectedPaymentMethod === "creditcard" ? "bg-accent" : ""}`} on:click={() => (selectedPaymentMethod = "creditcard")}>
                  <div class="flex items-center space-x-4">
                    <Wallet class="h-5 w-5" />
                    <div>
                      <h3 class="text-sm font-medium">Credit Card</h3>
                      <p class="text-muted-foreground text-sm">Pay with Visa, Mastercard, or American Express</p>
                    </div>
                  </div>
                  <RadioGroupItem value="creditcard" id="creditcard" />
                </div>
                <div class={`hover:bg-accent flex cursor-pointer items-center justify-between rounded-lg border p-4 ${selectedPaymentMethod === "creditcard2" ? "bg-accent" : ""}`} on:click={() => (selectedPaymentMethod = "creditcard2")}>
                  <div class="flex items-center space-x-4">
                ...
                <div class={`hover:bg-accent flex cursor-pointer items-center justify-between rounded-lg border p-4 ${selectedPaymentMethod === "applepay" ? "bg-accent" : ""}`} on:click={() => (selectedPaymentMethod = "applepay")}>
                  <div class="flex items-center space-x-4">
                    <Apple class="h-5 w-5" />
                    <div>
            ...
            <InnerDialogFooter class="flex flex-col items-center justify-between space-y-2 border-t px-4 py-2 sm:flex-row sm:space-y-0 sm:space-x-2">
              <DialogClose asChild>
